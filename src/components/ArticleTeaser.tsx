...
export default function ArticleTeaser({ article }: any) {
  ...
  return (
    <Link
      ...
      className="group bg-secondary flex flex-col max-w-[100%] w-[200px] sm:w-[276px] min-h-[355px] h-full rounded-xl border border-[#E9E9E9] dark:border-[#393939] hover:border-[#d3d2d2] dark:hover:border-[#575757] shadow-light hover:shadow-md"
      target="_blank"
    >
      <Image
        src={image?.url}
      ...
      <div className="flex flex-col grow justify-between p-4">
        <p className="font-bold text-sm xl:text-base mb-2 sm:mb-3 text-black dark:text-white/95">
          {article.title}
        </p>
        <p className="text-xs xl:text-sm text-black/60 dark:text-white/55">
          {short_teaser}
        </p>
      </div>
    </Link>
